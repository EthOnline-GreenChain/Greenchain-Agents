...
def build_agent(name: str, seed: str) -> Agent:
    ...
    inventory: Dict[str, Lot] = {
        "NP-NATURE-2022": Lot(
            provider_id=agent.address, credit_id="NP-NATURE-2022", registry="Verra",
            project_id="P-NP-001", project_class="nature_based", country="NP",
            vintage=2022, available_tonnes=5.0, unit_price_pyusd=9.9, min_lot_tonnes=0.08,
            co_benefits_rating=0.7, risk_delivery=0.06
        ),
    }
    ...
    async def handle_quote(ctx: Context, sender: str, msg: QuoteRequest):
        ...
        ctx.logger.info(f"[{name}] Quote -> {len(lots)} lot(s)")
