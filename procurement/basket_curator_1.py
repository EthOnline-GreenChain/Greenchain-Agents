...
# ---------- Intake protocol for external FootprintIntent ----------

INTENT_PROTOCOL = Protocol("footprint_intent.v1")

# protocol digest (string or 'proto:<hex>')
_digest = INTENT_PROTOCOL.digest
FOOTPRINT_PROTOCOL_DIGEST = _digest if isinstance(_digest, str) else _digest.hex()

@INTENT_PROTOCOL.on_message(model=FootprintIntent, allow_unverified=True)  # ← allow HTTP/unsigned
async def on_footprint_intent(ctx: Context, sender: str, intent: FootprintIntent):
    buyer_wallet = ctx.storage.get("buyer_wallet")
    await kickoff_procurement(ctx, intent, buyer_wallet=buyer_wallet)

# export the FootprintIntent **schema** digest (must come AFTER the handler)
FOOTPRINT_SCHEMA_DIGEST = next(
    (d for d, m in INTENT_PROTOCOL.models.items() if m is FootprintIntent), None
)
...
def build_agent(
    ...
    async def on_start(ctx: Context):
        ...
        ctx.storage.set("policy", policy or BALANCED_POLICY)  # Use default policy if none provided
        ctx.storage.set("last_trade", None)

        buyer_wallet = agent.address
        ctx.storage.set("buyer_wallet", buyer_wallet)

        ctx.logger.info(f"[{name}] built ok at {agent.address}")

        # CRITICAL FIX: Only run demo if demo_intent is provided
        if demo_intent is not None:
            ctx.logger.info(f"[{name}] Running demo intent with {demo_intent.tco2e} tCO2e")
            try:
                from procurement.basket_curator_1 import kickoff_procurement
                await kickoff_procurement(ctx, demo_intent, buyer_wallet=buyer_wallet)
            except Exception as e:
                ctx.logger.error(f"[{name}] kickoff failed: {e}")
        else:
            ctx.logger.info(f"[{name}] No demo intent - waiting for API triggers")
